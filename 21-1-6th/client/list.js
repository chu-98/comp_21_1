//리스트 페이지는 접속하자마자 함수가 실행되어야하므로 document.querySelector ~~~ 쓰지 않고, 함수를 만든 후 실행하는 방식으로 구현
...
async function getPost() { //아래에서 await을 써야하므로 async 함수로 만듦.
    try {
        ...
        postList.map((data) => { //map()이라는 함수는 배열에 사용. map()으로 DB에서 받아온 정보들을 하나 씩 가공하는 것.
                                 //()안에 콜백 함수를 넣어 어떤 동작을 할지 지정.
            ...
            //날짜 데이터를 입력하면 원하는 포맷으로 변경해줌.
            dataSpan.textContent = dayjs(dateTimeBefore).format('YYYY년 MM월 DD일 HH시 mm분');

            //appendChild()로 태그를 다른 태그 속에 넣을 수 있음.
            linkToDetail.appendChild(titleSpan); //제목을 게시글 상세페이지의 링크에 넣음 <a><span> 제목 </span></a> 이렇게 됩니다!
            postDiv.appendChild(linkToDetail);   //제목 + 링크를 상위 div에 담음.
            postDiv.appendChild(dateSpan);       //작성일을 상위 div에 담음.
            list.appendChild(postDiv);           //게시글 한 세트를 담은 상위 div를 list에 담음.

        })
    } catch(err) {
        console.log(err); //에러 발생 시 에러메세지를 보여줌.
    }
}
